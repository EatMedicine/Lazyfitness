...
{
    public class sendEmailCodeRegisterController : Controller
    {
        //盐因子
        ...
        public string sendVerification(string mailAddress)
        {
            ...
            Response.Cookies.Add(CookiesHelper.CookiesHelper.creatCookieMinutes("emailCode", encryptCode, 30));
            Response.Cookies.Add(CookiesHelper.CookiesHelper.creatCookieMinutes("emailAddress", encryptMailAddress, 30));
            try
            {
                //实例化一个发送邮件类。
                ...
                mailMessage.Body = "<div>尊敬的用户您好：<br/>欢迎注册小懒人健身，您的注册验证码为：<strong>" + code + "</strong>，请妥善保管。验证码30分钟内有效。</div>";
                //实例化一个SmtpClient类。
        ...
        {
            if (Request.Cookies["emailCode"] != null && Request.Cookies["emailAddress"] != null)
            {
                //获取Cookies的值
                HttpCookie cookieName = Request.Cookies["emailCode"];
                var rightCode = Server.HtmlEncode(cookieName.Value);

                HttpCookie cookieAddress = Request.Cookies["emailAddress"];
                ...
                //清空注册相关的cookies
                Response.Cookies.Add(CookiesHelper.CookiesHelper.clearCookie("emailCode"));
                Response.Cookies.Add(CookiesHelper.CookiesHelper.clearCookie("emailAddress"));
                return "T";
            }
            //验证码为空
            return "F";
