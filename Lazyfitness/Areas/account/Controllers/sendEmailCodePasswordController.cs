...
namespace Lazyfitness.Areas.account.Controllers
{
    public class sendEmailCodePasswordController : Controller
    {
        ...
        public string sendVerification(string mailAddress)
        {
            ...
            Response.Cookies.Add(CookiesHelper.CookiesHelper.creatCookieMinutes("emailAddressPassword", encryptMailAddress, 30));
            try
            {
                //实例化一个发送邮件类。
                ...
                mailMessage.Body = "<div>尊敬的<a href='http://www.lazyfitness.cn'>小懒人</a>用户您好：<br/>您的找回密码的验证码为：<strong>" + code + "</strong>，请妥善保管。验证码30分钟内有效。</div>";
                //实例化一个SmtpClient类。
        ...
        {
            if (Request.Cookies["emailCodePassword"] != null && Request.Cookies["emailAddressPassword"] != null)
            {
                //获取Cookies的值
